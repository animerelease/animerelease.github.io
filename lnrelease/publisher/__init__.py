...
from utils import EPOCH, SECONDARY, SOURCES, WORD_NUMBER, WORD_NUMBERS, Book, Format, Info, Series

NAME = 'misc'

# a volume value in a title: a digit form (1, 1.5, 1-A) or a spelled number
# ("Volume Eight"). The word branch is case-insensitive via an inline flag
# (PARSE/LOOSE are not IGNORECASE overall, and the digit branch's -[A-Z] must
# stay case-sensitive). normalize_vol() maps the word back to its digit.
_VOL = rf'(?P<volume>\d+(?:\.\d|-[A-Z])?|(?i:{WORD_NUMBER})\b)'
_LOOSE_VOL = rf'(?P<volume>\d+(?:\.\d+)?|(?i:{WORD_NUMBER})\b)'
PARSE = re.compile(r'(?P<name>.+?)(?:[,:]| [–-])? *(?:\bVol\.|(?:[\(\[]|\b)Volume|\(Manga) *0*' + _VOL + r'[\)\]]?(?:(?:[,:]|\s+[–-])? Part (?P<part>\S+)|\s*[:\(].+|\s+[–-]\D+)?')
...
LOOSE = re.compile(r'(?P<name>.+?)(?:,|:| [–-])? *(?:[\(\[]|\b)(?:Vol\.|Volume|Part|Manga) *' + _LOOSE_VOL + r'[\)\]]?(?:\s*[:–\-\(]?.+)?')
...
def normalize_vol(vol: str) -> str:
    """A captured volume token -> its digit form ("Eight" -> "8", "3" -> "3")."""
    return str(WORD_NUMBERS[vol.lower()]) if vol and vol.lower() in WORD_NUMBERS else vol
...
def diff_list(titles: list[str]) -> list[str]:
    if len(titles) == 0:
        ...
        if match := LOOSE.fullmatch(titles[0]):
            return [normalize_vol(match.group('volume'))]
        else:
            return ['']
    ...
    for key, lst in info.items():
        for i, inf in enumerate(lst):
            if match := PARSE.fullmatch(inf.title):
                ...
                vol = normalize_vol(match.group('volume'))
