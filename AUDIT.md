...
*Two left for review* (creator nationality not confidently derivable — kept at their existing `other/webtoon` rather than guessing): **Daybreak**, **GremoryLand**. — **Resolved 2026-07-17:** both classifications researched and confirmed at `other/webtoon`. *Daybreak* is by **Moosopp**, a WEBTOON creator based in Canada (at most Cambodian/Vietnamese heritage per their own social bios — neither JP/KR/CN, so origin `other`; webtoon-native in print, so category `webtoon`). *GremoryLand* is by **A. Rasen**, described in the volume's own author bio as "a comic creator from Barcelona, Spain" (Western → origin `other`; webtoon-native → category `webtoon`). No `origins.csv` change was needed — the existing entries were already correct — but the two are no longer provisional guesses.
